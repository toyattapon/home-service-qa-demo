...
export function DispatchPage() {
  ...
  async function assign() { try { await jobApi.assign(jobId, technicianId); setMessage('Assignment saved successfully'); setError(''); await load(); } catch (caught) { setError((caught as Error).message); setMessage(''); } }
  return (
    <>
      ...
      <section className="panel"><div className="toolbar">
        <label>Date filter<input data-testid="dispatch-date-input" type="date" value={date} onChange={(e) => setDate(e.target.value)} /></label>
        ...
        <button data-testid="dispatch-assign-button" className="button primary" disabled={!jobId || !technicianId} onClick={() => void assign()}>Assign</button>
